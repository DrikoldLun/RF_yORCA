...
Datapath='../../RF_data/CCP/RF50_0.10to0.50/RFdat_rmul'
...
[Stations Stalon Stalat]=textread(Stalist,'%s %f %f %*s %*s %*s %*s',-1);
...
filename=['pierce0.1-0.5hz.dat'];xst=exist(filename,'file');fid=fopen(filename,'w+');
RFdepth = struct([]);
...
save RFdepth_rwater_newsta.mat RFdepth
fclose(fid);
