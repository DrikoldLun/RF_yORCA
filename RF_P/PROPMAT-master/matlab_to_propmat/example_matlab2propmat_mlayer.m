...
modCA_ZK2000 = struct('nlay',4,...
                      'zlayt',[0 5.5 16.0 32.0]',...
                      'zlayb',[  5.5 16.0 32.0 100]',...
                      'Vs',   [3.28 3.64 3.67 4.50]');
modCA_ZK2000.Vp = [1.73 1.731 1.731 1.733]'.*modCA_ZK2000.Vs;
modCA_ZK2000.rho = [sed_vs2rho(modCA_ZK2000.Vs(1:3));mantle_vs2rho(modCA_ZK2000.Vs(end),20)];
...
%% use PropMAT to model Sp arrivals
samprate = 25;
inc = 25;
rayp = sind(inc)./modCA_ZK2000.Vp(end);
[trS,ttS] = run_propmat(modCA_ZK2000,[],'Sp',samprate,inc,synthperiod);
% output is TRZ, switch to ZRT, normalise
dataS = trS(:,[3,1,2])./maxab(maxab(trS));

% some info about traces - same for P or S
nsamps = length(ttS);
dt = diff(ttS(1:2));
nchans = size(trS,2);
...
ttP = ttP - t_arP;
t_arS =  ttS(dataS(:,2)==max(abs(dataS(:,2))));
ttS = ttS - t_arS;
...
for ic = 1:3
    ...
    % S
    subplot(3,2,2*ic)
    plot(ttS,dataS(:,ic),'r','linewidth',2)

    set(gca,'ylim',[-1 1],'xlim',[-30 30],'fontsize',13)
    ylabel(chans{ic},'fontsize',20)
    if ic == 3, xlabel('Time since S arrival (s)','fontsize',20); end
    if ic == 1, title('S-wave (s)','fontsize',25); end


end

result = py.tr.Ps_t_amp(modCA_ZK2000.Vp,modCA_ZK2000.Vs,modCA_ZK2000.zlayb-modCA_ZK2000.zlayt,modCA_ZK2000.rho,deg2rad(inc),2);
T1 = 0.5;
T2 = 10;
...
R_sem = load('semd_mlayer/AA.S0006.BXX.semd');
Z_sem = load('semd_mlayer/AA.S0006.BXZ.semd');
...
subplot(3,1,3)
plot(result{'t'}{i}-result{'t'}{1},0,'ko')
end
return
