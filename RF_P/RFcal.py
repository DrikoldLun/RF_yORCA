...
# ------- Filtering and culling based on snr ------- #
        ...
        if np.mod(fs,samprate) != 0:
            ...
            logger.warning(msg)
        st_tmp.decimate(factor=int(fs//samprate),no_filter=True,strict_length=True)
        snr[suffix[1]] = geo.snr(st_tmp.data, samprate, t_P-5, 50, 15)
...
# ------- windowing ------- #
    ...
    winb, wine = int((t_P - RF_calcwinlen)*samprate), int((t_P + RF_calcwinlen)*samprate)
...
# ------- calculate RF ------- #
    ...
    rf_R_t = decov.decovt(Z,R,1./samprate,time_beforeP,time_afterP,gauss)
    rf_T_t = decov.decovt(Z,T,1./samprate,time_beforeP,time_afterP,gauss)
    ...
    st_rf_r = st_r.copy()
    st_rf_t = st_t.copy()
    for st_tmp,RF,suffix in zip([st_rf_r,st_rf_t],[rf_R,rf_T],['.R','.T']):
        st_tmp.stats.sac.user0 = gauss
        st_tmp.data = RF
        stsac = SACTrace.from_obspy_trace(st_tmp)
        stsac.reftime = st_tmp.stats.starttime+time_beforeP
        st_tmp = stsac.to_obspy_trace()
        st_tmp.write(os.path.join(RF_path,prefix+'_PRF'+suffix),'SAC')
    ...
    ax = []
    for i in range(1,5):
        ax.append(fig.add_subplot(4,1,i))
    RFtime=np.arange(-time_beforeP,time_afterP+1./samprate,1./samprate)
    ax[0].plot(np.arange(0,len(Z)),Z)
    ax[1].plot(np.arange(0,len(R)),R)
    ax[2].plot(RFtime,rf_R_t)
    ax[3].plot(RFtime,rf_R_f)
    plt.show()
    '''
