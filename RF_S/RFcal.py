...
# ------- Read input ------- #
def Usage():
    msg = 'python RFcal.py -Sstation [-O](reorientation?) [-F](frequency domain deconvolution) [-D](after sensorball drop) para.cfg'
    logger.info(msg)
    sys.exit(1)
...
rotate_path = config.get('path','rotate_dir')
rotate_path = os.path.join(out_path,rotate_path,staname)
...
time_beforeS = config.getfloat('para','time_beforeS') 
time_afterS = config.getfloat('para','time_afterS')
...
# ------- Extract info ------- #
    ...
    try:
        arrivals = model.get_travel_times(source_depth_in_km=stats.evdp,distance_in_degree=gcarc,phase_list=['S'])
        t_S = arrivals[0].time
        rayp = geo.srad2skm(arrivals[0].ray_param)

    except:
        msg = "no theoretical S arrival, continue"
        logger.info(msg)
        continue
...
# ------- windowing ------- #
    ...
    winb, wine = int((t_S - RF_calcwinl)*samprate), int((t_S + RF_calcwinr)*samprate)
    Z = st_z.data[winb:wine]
    R = st_r.data[winb:wine]
    T = st_t.data[winb:wine]

# ------- calculate RF ------- #

    rf_Z_t = decov.decovt(R,Z,1./samprate,time_beforeS,time_afterS,gauss)
    rf_T_t = decov.decovt(R,T,1./samprate,time_beforeS,time_afterS,gauss)

    #rf_R_f = decov.decovf(Z,R,1./samprate,RF_calcwinlen,time_beforeP,time_afterP,gauss)
    #rf_T_f = decov.decovf(Z,T,1./samprate,RF_calcwinlen,time_beforeP,time_afterP,gauss)
   
    rf_Z = rf_Z_t
    rf_T = rf_T_t
    #if not fre_decov:
    #    rf_R, rf_T = rf_R_t, rf_T_t
    #else:
    #    rf_R, rf_T = rf_R_f, rf_T_f
    st_rf_z = st_z.copy()
    st_rf_t = st_t.copy()
    for st_tmp,RF,suffix in zip([st_rf_z,st_rf_t],[rf_Z,rf_T],['.Z','.T']):
        st_tmp.stats.sac.user0 = gauss
        st_tmp.data = RF
        stsac = SACTrace.from_obspy_trace(st_tmp)
        stsac.reftime = st_tmp.stats.starttime+time_beforeS
        st_tmp = stsac.to_obspy_trace()
        st_tmp.write(os.path.join(RF_path,prefix+'_SRF'+suffix),'SAC')
    ...
    trtime = np.linspace(-RF_calcwinl,RF_calcwinr,num=len(Z))
    RFtime = np.linspace(-time_beforeS,time_afterS,num=len(rf_Z_t))
    ax[0].plot(trtime,Z)
    ax[1].plot(trtime,R)
    ax[2].plot(RFtime,rf_Z_t)
    plt.suptitle('dist=%.1f'%(st_rf_z.stats.sac.gcarc))
    plt.show()
    plt.close('all') 
    '''
