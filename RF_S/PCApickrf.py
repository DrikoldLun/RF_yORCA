...
def Usage():
    print("Usage:  python pickrf.py -Sstation -C[R/T]comp  para.cfg")
    sys.exit(1)
...
def get_sac():
    ...
    time_before = config.getint('para', 'time_beforeS') * -1
    time_after = config.getint('para', 'time_afterS')
    path = os.path.join(path,station)
    filenames = []
    for filename in glob.glob(os.path.join(path, '*_SRF.'+comp)):
        filenames.append(os.path.basename(filename).split('_')[0])
    filenames.sort()
    rffiles = obspy.read(os.path.join(path, '*_SRF.'+comp))
...
class plotrffig():
    ...
    def finish(self, event):
        ...
        if not os.path.exists('PCA/evtlog'):
            os.makedirs('PCA/evtlog')
        lines = [[], []]
        for i in range(opts.evt_num):
            lines[self.goodrf[i]].append('%s\n'%(os.path.basename(opts.filenames[i])[:12]))
        with open('PCA/evtlog/'+opts.staname+'_goodRF.lst','w+') as f:
            f.writelines(lines[1])
            f.close()
        with open('PCA/evtlog/'+opts.staname+'_badRF.lst','w+') as f:
            f.writelines(lines[0])
            f.close()
        
        stack = np.zeros(opts.RFlength)
        with open(os.path.join('PCA/evtlog', opts.staname+"finallist.dat"), 'w+') as fid:
            for i in range(opts.evt_num):
                evtname = os.path.basename(opts.filenames[i])
                if self.goodrf[i] == 0:
                    #os.system('rm '+os.path.join(opts.path,evtname+'*SRF.*'))
                    print("Reject SRF of "+evtname)
                    continue        
        ...
        np.savetxt('PCA/evtlog/'+opts.staname+'_PCA.dat',np.vstack((time_axis,stack)),fmt='%12.7f',newline='\n')
        sys.exit(0)
    ...
    def plot(self, event):
        ...
        plotrf.classic_plot(st, opts.b, opts.e, opts.image_path)
    ...
    def ampcull(self, event):
        ...
        for i in range(opts.evt_num):
            if self.goodrf[i] == 1:
                Pamp = opts.rffiles[i].data[np.where((time_axis>-0.2)&(time_axis<0.2))].max()
