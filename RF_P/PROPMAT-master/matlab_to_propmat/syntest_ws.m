...
clear all
moho = [6 5 5 6.5 6 6 6];
sed = [0 0.2 0 0 0 0 0.2];
%sed = [0 0 0 0 0 0 0];
sedVs = [1 1 1 1 1 1 0.7];
sta = {'CC02' 'CC04' 'CC05' 'WC03' 'CC07' 'CC08' 'WC04'};
% CC04
% close all

%% CA structure - match Zhu and Kanamori 2000
for i = 1:length(moho)
modCA_ZK2000 = struct('nlay',3,...
                      'zlayt',[0 sed(i) moho(i)]',...
                      'zlayb',[sed(i) moho(i) 30.]',...
                      'Vs',   [sedVs(i) 3.5 4.1]');
modCA_ZK2000.Vp = [1.5 6. 7.2]';
modCA_ZK2000.rho = [2 2.72 3.03]';
...
%RF_stack_dir = 'RFstack/RF50_0.10to2.00';
%RF_stack_dir = 'RFstack/RF50_0.10to1.00';
%RF_stack_dir = 'PCAstack/RF50_0.10to2.00';
RF_stack_dir = '../../PCA/evtlog/RF50_0.10to2.00/';
%RF_stack_path = fullfile(RF_stack_dir,[sta{i},'_stack.dat']);
RF_stack_path = fullfile(RF_stack_dir,['XX.',sta{i},'_PCA.dat']);
...
R_pm = cutP(R_pm,amp_pm,samprate,5,15);
Z_pm = cutP(Z_pm,amp_pm,samprate,5,15);
alpha = 5;
...
plotZR(RF_pm,1,[4,2],i,samprate,2,10,'red',[sta{i},' (syn\_Moho=',num2str(moho(i)),',sedh=',num2str(sed(i)),',sedVs=',num2str(sedVs(i)),')']);
...
plot(stack_t,stack_amp,'color','black','linewidth',1.5)
xlim([-2 10])
legend('Syn','Obs')
%{
for i = 1:4
...
set(gcf,'Position',[100 600 1500 900])
%% ---------------------------- SUBFUNCTIONS ----------------------------
%% ---------------------------- SUBFUNCTIONS ----------------------------
%% ---------------------------- SUBFUNCTIONS ----------------------------
